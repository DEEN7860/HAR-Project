...
OUTPUT_DIR = Path("outputs_knn") # Changed for KNN
...
def build_dataset():
    ...
    for class_dir in sorted(DATA_ROOT.iterdir()):
        ...
        for zip_path in sorted(class_dir.glob("*.zip")):
            session_id = f"{label}___knn__{zip_path.stem}"
...
def train_and_evaluate(df_feat: pd.DataFrame):
    ...
    # Save artifacts to KNN folder
    model_path = OUTPUT_DIR / "har_knn.joblib"
    joblib.dump(clf, model_path)

    features_path = OUTPUT_DIR / "feature_columns.txt"
    features_path.write_text("\n".join(X.columns))

    print(f"\nSaved KNN model to: {model_path}")
...
if __name__ == "__main__":
    ...
    df_feat.to_csv(OUTPUT_DIR / "features_dataset_knn.csv", index=False)
