...
OUTPUT_DIR = Path("outputs_svm") # Changed for SVM
...
def train_and_evaluate(df_feat: pd.DataFrame):
    ...
    # Use a Scaler + SVM Pipeline
    clf = make_pipeline(
        StandardScaler(), 
        SVC(kernel='rbf', class_weight='balanced', random_state=RANDOM_STATE)
    )

    print("\nTraining SVM... This might take a moment depending on dataset size.")
    ...
    # Save artifacts to SVM folder
    model_path = OUTPUT_DIR / "har_svm.joblib"
...
if __name__ == "__main__":
    ...
    df_feat.to_csv(OUTPUT_DIR / "features_dataset_svm.csv", index=False)
...
# 1. Load the dataset (Make sure this path is correct)
df = pd.read_csv(r'C:\Users\deenm\OneDrive\Documents\har_project\outputs_svm\features_dataset_svm.csv')

# 2. Separate Features (X) and Labels (y)
# We drop 'label' and 'group' (if it exists) to get just the math features
X = df.drop(columns=['label', 'group'], errors='ignore')
y = df['label']

# 3. Split into 70% Training / 30% Testing
X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.3, random_state=42)

# 4. Scale the data (Crucial for SVM!)
...
# 6. Train the SVM (RBF Kernel)
print("Training SVM...")
svm = SVC(kernel='rbf', random_state=42)
svm.fit(X_train_scaled, y_train)
y_pred_svm = svm.predict(X_test_scaled)

classes = ['running', 'sitting', 'standing', 'walking']
...
ax1.set_title('Random Forest Baseline')
...
disp_svm.plot(ax=ax2, cmap='Greens', colorbar=False)
ax2.set_title('SVM (RBF Kernel)')
